...
var _new = function(args){
  ...
  if (fs.existsSync(path.join(current_directory, name))) {
    return console.error((name + ' folder already exists!').red);
  }

  var template_name = args.template || "default";
  ...
  shell.cp('-r', source + '/.*', destination); // ugly hack to also copy over .gitignore (which annoyingly gets renamed to .npmignore by npm) and other dotfiles
  if (!fs.existsSync(destination + "/.gitignore")) {
    shell.mv(destination + "/.npmignore", destination + "/.gitignore");
  }

  // add current lfa version to app.coffee
  var app_config_path = path.join(destination, 'app.coffee')
  fs.existsSync(app_config_path) && add_lfa_version(app_config_path)
...
function add_lfa_version(app_config_path){
  ...
  fs.writeFileSync(app_config_path, "# lfa v" + current_version + '\n' + app_config);
}

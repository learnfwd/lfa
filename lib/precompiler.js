  ...
  */
  ...
  Precompiler.prototype.parseTemplateFileName = function(template) {
    ...
    return {
      ...
      basePath: basePath,
      fileName: template,
      isChapter: template.indexOf(path.join(process.cwd(), global.options.folder_config.views)) === 0
    };
  };
  ...
  */
  ...
  Precompiler.prototype.compileTemplate = function(opts) {
    ...
    if (opts.isChapter) {
      ...
      data = ['function () { return ', JSON.stringify(html), '; }'].join('');

      toc_generator.add_toc_item(opts.basePath, locals);
      toc_generator.add_search_item(opts.basePath, locals, html);
    }
    
    var buf = [(opts.isChapter ? 'asyncDefine(\'' + opts.namespace + '\', ' : 'define('), 'function(){ return ', data, '});'].join('');

...
module.exports = function(file){
  ...
  function parse_dynamic_content(){
    ...
    if (front_matter_string) {
      ...
      if (options.folder_config.layouts === 'templates/layouts') {
        ...
        f.layout_path = path.join(__dirname, '..' + path.sep + '..' + path.sep + options.folder_config.layouts, options.layouts.default);
      } else {
        // TODO: implement custom layouts
      }
      f.layout_contents = fs.readFileSync(f.layout_path, 'utf8');
      f.dynamic_locals.url = f.path.replace(path.join(process.cwd(), global.options.folder_config.views) + path.sep, '').replace(/\..*$/, '.html').replace('\\', '/');
  ...
  function set_layout(){
    ...
    if (layouts_set && !f.dynamic_locals) {
      ...
      if (options.folder_config.layouts === 'templates/layouts') {
        // we're using the default layouts
        f.layout_path = path.join(__dirname, '..' + path.sep + '..' + path.sep + options.folder_config.layouts, layout);
      } else {
        // TODO: implement custom layouts
      }
      f.layout_contents = fs.readFileSync(f.layout_path, 'utf8');
  ...
  function locals(extra){
    ...
    locals.path_to_root = (function() {
      ...
      var path_to_root = "";
      if (diff.length) {
        for (var i = 0; i < (diff.split(path.sep).length - 1); i++) {
          path_to_root += "../";
        }
      }
      return path_to_root;

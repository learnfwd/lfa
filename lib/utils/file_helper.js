...
module.exports = function(file){
  ...
  function parse_dynamic_content(){
    ...
    if (front_matter_string) {
      ...
      if (front_matter.layout) {
        // if the layout key in front-matter does not end with jade
        if (!(/jade$/.test(front_matter.layout))) {
          // lookup the key in the layouts map
          front_matter.layout = options.layouts[front_matter.layout];
        }
      }
      // set the layout to the default one if it's undefined
      front_matter.layout = front_matter.layout || options.layouts.default;
      
      // TODO: figure out why this thing has to be /layouts/layouts
      f.layout_path = path.join(process.cwd(), options.folder_config.layouts, front_matter.layout);
  ...
  function set_layout(){
    ...
    if (layouts_set && !f.dynamic_locals) {
      ...
      f.layout_path = path.join(process.cwd(), options.folder_config.layouts, layout);
      f.layout_contents = fs.readFileSync(f.layout_path, 'utf8');

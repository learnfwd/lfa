...
function create_toc(root) {
  global.options.toc = treeToc(path.join(root, global.options.folder_config.views), 0);
  
  function treeToc(p, depth) {
    ...
    files.forEach(function(file) {
      ...
      if (fs.lstatSync(newpath).isFile()) {
        children.push({ fm: yaml_parser.readParse(newpath), children: null });
      } else {
        children.push(treeToc(newpath, depth + 1));
      }
    });
    
    if (depth) { // if we're no longer at the base level
      var result = { fm: yaml_parser.readParse(path.join(p, firstFile)), children: children }
    } else {
      var result = children;
    }

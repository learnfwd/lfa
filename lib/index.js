...
function compile(ast){
  ...
  if (global.options.components == "false") {
    global.options.debug.log('skipping component library inclusion');
  } else {
    global.options.debug.log('adding component library');
    shell.cp(
      '-rf', 
...
function create_folders(ast){
  ...
  for (var key in ast.folders) {
    ...
    if (_.contains(folders, global.options.folder_config.views) && folders.length > 2) {
      global.options.debug.log("skipping " + ast.folders[key].replace(process.cwd(), ''));
      continue;
    }
